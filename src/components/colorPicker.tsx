...
function ColorPicker(props: any) {
  ...
  useEffect(() => {
    const clickAway =  (event:any) => {
      if(props.showColor[props.code] && !colorRef.current.contains(event.target)){
  ...
  return (
    <div ref = {colorRef} style={{zIndex: "4"}} onMouseLeave={()=> {props.handleColorView(props.code, false)}}>
